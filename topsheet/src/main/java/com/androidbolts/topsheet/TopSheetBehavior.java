...
public class TopSheetBehavior<V extends View> extends CoordinatorLayout.Behavior<V>{
    ...
    public abstract static class TopSheetCallback {
         ...
         */
        public abstract void onStateChanged(@NonNull View bottomSheet, @TopSheetBehavior.State int newState);
    ...
    private TopSheetBehavior.SettleRunnable settleRunnable = null;
    ...
    private static ColorStateList getColorStateList(@NonNull Context context, @NonNull TypedArray attributes, @StyleableRes int index) {
    ...
    public TopSheetBehavior(@NonNull Context context, @Nullable AttributeSet attrs) {
        ...
        // Only with higher material component versions
...
//        setDraggable(a.getBoolean(R.styleable.BottomSheetBehavior_Layout_behavior_draggable, true));
    ...
    public Parcelable onSaveInstanceState(@NonNull CoordinatorLayout parent, @NonNull V child) {
        return new TopSheetBehavior.SavedState(super.onSaveInstanceState(parent, child), this);
    }

    @Override
    public void onRestoreInstanceState(
            @NonNull CoordinatorLayout parent, @NonNull V child, @NonNull Parcelable state) {
        TopSheetBehavior.SavedState ss = (TopSheetBehavior.SavedState) state;
    ...
    public boolean onLayoutChild(
        ...
        if (state == STATE_EXPANDED) {
            ViewCompat.offsetTopAndBottom(child, transformOffset(child, getExpandedOffset()));
        } else if (state == STATE_HALF_EXPANDED) {
            ViewCompat.offsetTopAndBottom(child, transformOffset(child, halfExpandedOffset));
        } else if (hideable && state == STATE_HIDDEN) {
            ViewCompat.offsetTopAndBottom(child, transformOffset(child, parentHeight));
        } else if (state == STATE_COLLAPSED) {
            ViewCompat.offsetTopAndBottom(child, transformOffset(child, collapsedOffset));
        } else if (state == STATE_DRAGGING || state == STATE_SETTLING) {
            ViewCompat.offsetTopAndBottom(child, transformOffset(child, savedTop - child.getTop()));
        }
    ...
    private int transformOffset(View child, int offset) {
        return -1 * offset + parentHeight - child.getHeight();
    }
    ...
    public void onNestedPreScroll(
        ...
        if (dy > 0) { // Upward
            if (!target.canScrollVertically(-1)) {
                if (newTop <= transformOffset(child, collapsedOffset) || hideable) {
                    if (!draggable) {
                        // Prevent dragging
                        return;
                    }
                    ...
                    consumed[1] = currentTop - transformOffset(child, collapsedOffset);
                    ViewCompat.offsetTopAndBottom(child, -consumed[1]);
                    setStateInternal(STATE_COLLAPSED);
                }
            }
        } else if (dy < 0) { // Downward
            if (newTop < transformOffset(child, getExpandedOffset())) {
                consumed[1] = currentTop - transformOffset(child, getExpandedOffset());
                ViewCompat.offsetTopAndBottom(child, -consumed[1]);
    ...
    public void onStopNestedScroll(
            ...
            @NonNull View target,
            int type) {
        if (child.getTop() == transformOffset(child, getExpandedOffset())) {
            setStateInternal(STATE_EXPANDED);
            return;
        }
        ...
        if (lastNestedScrollDy > 0) {
            if (fitToContents) {
                top = transformOffset(child, fitToContentsOffset);
                targetState = STATE_EXPANDED;
            } else {
                int currentTop = child.getTop();
                if (currentTop > transformOffset(child, halfExpandedOffset)) {
                    top = transformOffset(child, halfExpandedOffset);
                    targetState = STATE_HALF_EXPANDED;
                } else {
                    top = transformOffset(child, expandedOffset);
                    targetState = STATE_EXPANDED;
                }
            }
        } else if (hideable && shouldHide(child, getYVelocity())) {
            top = transformOffset(child, parentHeight);
            targetState = STATE_HIDDEN;
        } else if (lastNestedScrollDy == 0) {
            int currentTop = child.getTop();
            if (fitToContents) {
                if (Math.abs(currentTop - transformOffset(child, fitToContentsOffset)) < Math.abs(currentTop - transformOffset(child, collapsedOffset))) {
                    top = transformOffset(child, fitToContentsOffset);
                    targetState = STATE_EXPANDED;
                } else {
                    top = transformOffset(child, collapsedOffset);
                    targetState = STATE_COLLAPSED;
                }
            } else {
                if (currentTop < transformOffset(child, halfExpandedOffset)) {
                    if (currentTop < Math.abs(currentTop - transformOffset(child, collapsedOffset))) {
                        top = transformOffset(child, expandedOffset);
                        targetState = STATE_EXPANDED;
                    } else {
                        top = transformOffset(child, halfExpandedOffset);
                    ...
                    if (Math.abs(currentTop - transformOffset(child, halfExpandedOffset)) < Math.abs(currentTop - transformOffset(child, collapsedOffset))) {
                        top = transformOffset(child, halfExpandedOffset);
                        targetState = STATE_HALF_EXPANDED;
                    } else {
                        top = transformOffset(child, collapsedOffset);
     ...
     */
    public void setSaveFlags(@TopSheetBehavior.SaveFlags int flags) {
     ...
     * @deprecated use {@link #addBottomSheetCallback(TopSheetCallback)} and {@link
     *     #removeBottomSheetCallback(TopSheetCallback)} instead
     */
    @Deprecated
    public void setBottomSheetCallback(TopSheetCallback callback) {
        Log.w(
                TAG,
                "TopSheetBehavior now supports multiple callbacks. `setBottomSheetCallback()` removes"
     ...
     * @param callback The callback to notify when bottom sheet events occur.
     */
    public void addBottomSheetCallback(@NonNull TopSheetCallback callback) {
     ...
     */
    public void removeBottomSheetCallback(@NonNull TopSheetCallback callback) {
     ...
     */
    public void setState(@TopSheetBehavior.State int state) {
        if (state == this.state) {
            return;
        }
        if (viewRef == null) {
    ...
    private void settleToStatePendingLayout(@TopSheetBehavior.State int state) {
    ...
    void setStateInternal(@TopSheetBehavior.State int state) {
        if (this.state == state) {
            return;
        }
        this.state = state;
    ...
    private void updateDrawableForTargetState(@TopSheetBehavior.State int state) {
        if (state == STATE_SETTLING) {
    ...
    private void restoreOptionalState(@NonNull TopSheetBehavior.SavedState ss) {
    ...
    boolean shouldHide(@NonNull View child, float yvel) {
        ...
        if (child.getTop() > transformOffset(child, collapsedOffset)) {
            // It should not hide, but collapse.
            return false;
        }
        int peek = calculatePeekHeight();
        final float newTop = child.getTop() + yvel * HIDE_FRICTION;
        return Math.abs(transformOffset(child, (int)newTop) - collapsedOffset) / (float) peek > HIDE_THRESHOLD;
    }
    ...
    void settleToState(@NonNull View child, int state) {
        ...
        if (state == STATE_COLLAPSED) {
            ...
            top = transformOffset(child, getExpandedOffset());
        } else if (hideable && state == STATE_HIDDEN) {
            top = transformOffset(child, parentHeight);
        } else {
    ...
    void startSettlingAnimation(View child, int state, int top, boolean settleFromViewDragHelper) {
        ...
        if (startedSettling) {
            ...
            if (settleRunnable == null) {
                ...
                settleRunnable = new TopSheetBehavior.SettleRunnable(child, state);
            }
    ...
    private final ViewDragHelper.Callback dragCallback =
            new ViewDragHelper.Callback() {
                ...
                private boolean releasedLow(@NonNull View child) {
                    ...
                    return child.getTop() < transformOffset(child, (parentHeight + getExpandedOffset()) / 2);
                }

                @Override
                public void onViewReleased(@NonNull View releasedChild, float xvel, float yvel) {
                    int top;
                    @TopSheetBehavior.State int targetState;
                    if (yvel > 0) { // Moving down
                        if (fitToContents) {
                            top = transformOffset(releasedChild, fitToContentsOffset);
                            targetState = STATE_EXPANDED;
                        } else {
                            int currentTop = releasedChild.getTop();
                            if (currentTop > transformOffset(releasedChild, halfExpandedOffset)) {
                                top = transformOffset(releasedChild, halfExpandedOffset);
                                targetState = STATE_HALF_EXPANDED;
                            } else {
                                top = transformOffset(releasedChild, expandedOffset);
                                targetState = STATE_EXPANDED;
                            }
                        }
                        ...
                        if ((Math.abs(xvel) < Math.abs(yvel) && Math.abs(yvel) < SIGNIFICANT_VEL_THRESHOLD)
                                || releasedLow(releasedChild)) {
                            top = transformOffset(releasedChild, parentHeight);
                            targetState = STATE_HIDDEN;
                        } else if (fitToContents) {
                            top = transformOffset(releasedChild, fitToContentsOffset);
                            targetState = STATE_EXPANDED;
                        } else if (Math.abs(releasedChild.getTop() - transformOffset(releasedChild, expandedOffset))
                                < Math.abs(releasedChild.getTop() - transformOffset(releasedChild, halfExpandedOffset))) {
                            top = transformOffset(releasedChild, expandedOffset);
                            targetState = STATE_EXPANDED;
                        } else {
                            top = transformOffset(releasedChild, halfExpandedOffset);
                        ...
                        int currentTop = releasedChild.getTop();
                        if (fitToContents) {
                            if (Math.abs(currentTop - transformOffset(releasedChild, fitToContentsOffset))
                                    < Math.abs(currentTop - transformOffset(releasedChild, collapsedOffset))) {
                                top = transformOffset(releasedChild, fitToContentsOffset);
                                targetState = STATE_EXPANDED;
                            } else {
                                top = transformOffset(releasedChild, collapsedOffset);
                                targetState = STATE_COLLAPSED;
                            }
                        } else {
                            if (currentTop > transformOffset(releasedChild, halfExpandedOffset)) {
                                if (currentTop < Math.abs(currentTop - transformOffset(releasedChild, collapsedOffset))) {
                                    top = transformOffset(releasedChild, expandedOffset);
                                    targetState = STATE_EXPANDED;
                                } else {
                                    top = transformOffset(releasedChild, halfExpandedOffset);
                                ...
                                if (Math.abs(currentTop - transformOffset(releasedChild, halfExpandedOffset))
                                        < Math.abs(currentTop - transformOffset(releasedChild, collapsedOffset))) {
                                    top = transformOffset(releasedChild, halfExpandedOffset);
                                    targetState = STATE_HALF_EXPANDED;
                                } else {
                ...
                public int clampViewPositionVertical(@NonNull View child, int top, int dy) {
                    return MathUtils.clamp(
                            top, hideable ? transformOffset(child, parentHeight) : transformOffset(child, collapsedOffset), transformOffset(child, getExpandedOffset()));
                }
    ...
    void dispatchOnSlide(int top) {
        ...
        if (bottomSheet != null && !callbacks.isEmpty()) {
            final int transformedCollapseOffset = transformOffset(bottomSheet, collapsedOffset);
            float slideOffset =
                    (top > transformedCollapseOffset || collapsedOffset == getExpandedOffset())
                            ? (float) (top - transformedCollapseOffset) / (parentHeight - collapsedOffset)
                            : (float) (top - transformedCollapseOffset) / (collapsedOffset - getExpandedOffset());
            for (int i = 0; i < callbacks.size(); i++) {
    ...
    private class SettleRunnable implements Runnable {
        ...
        SettleRunnable(View view, @TopSheetBehavior.State int targetState) {
            this.view = view;
    ...
    protected static class SavedState extends AbsSavedState {
        ...
        public static final Creator<TopSheetBehavior.SavedState> CREATOR =
                new ClassLoaderCreator<TopSheetBehavior.SavedState>() {
                    @NonNull
                    @Override
                    public TopSheetBehavior.SavedState createFromParcel(@NonNull Parcel in, ClassLoader loader) {
                        return new TopSheetBehavior.SavedState(in, loader);
                    }

                    @Nullable
                    @Override
                    public TopSheetBehavior.SavedState createFromParcel(@NonNull Parcel in) {
                        return new TopSheetBehavior.SavedState(in, null);
                    }
